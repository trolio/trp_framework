...
AddEventHandler('playerDropped', function()
    ...
    if (Users[Source]) then
        TriggerEvent("trp:playerDropped", Users[Source])
        db.udpateUser(Users[Source].get('identifier'), {money = Users[Source].getMoney(), bank = Users[Source].getBank()})
        log('User (' .. Users[Source].get(Users[Source].getSessionVar('idType')).. ') saved and unlocked')
        Users[Source] = nil
    end
end)
...
AddEventHandler('playerConnecting', function(name, setKickReason)
    ...
    for k, v ipairs(GetPlayerIdentifiers(source)) do
        if sting.sub(v, 1, string.len("steam:")) == "steam:" then
            id = v
            break
        end
    end
...
RegisterServerEvent('trp:firstJoinedProper')
AddEventHandler('trp:firstJoinedProper', function()
    local Source = source
    ...
    Citizen.CreateThread(function()
        ...
        if not id then
            ...
            if (settings.defaultSettings.pvpEnabled) then
                TriggerClientEvent("trp:enablePVP", Source)
            end
        end
        return
    end)
...
AddEventHandler('trp:setSessionSetting', function(k, v)
    settings.sessionSettings[k] = value
end)

AddEventHandler('trp:getSessionSetting', function(k, cb)
...
AddEventHandler('playerSpawn', function()
    ...
    if (firstSpawn[Source]) == nil) then
        Citizen.CreateThread(function()
            ...
            TriggerEvent("trp:firstSpawn", Source, Users[Source])
...
AddEventHandler("trp:setDefaultSettings", function(tbl)
...
AddEventHandler('chatMessage', function(source, n, message)
    ...
    if (startswith(message, settings.defaultSettings.commandDelimeter)) then
            ...
            if(command.perm > 0) then
                if(IsPlayerAceAllowed(Source, "command." .. command_args[1]) or Users[source].getPermissions() >= command.perm or groups[Users[source].getGroup()]:canTarget(command.group)) then
                    ...
                    if (not (command.arguments == #command_args - 1) and command.arguments > -1) then
                        TriggerEvent("trp:incorrectAmountOfArguments", source, commands[command].arguments, #args, Users[source])
                    else
                        command.cmd(source, command_args, Users[source])
                        TriggerEvent("trp:adminCommandRan", source, command_args, Users[source])
                    ...
                    TriggerEvent("trp:adminCommandFailed", source, command_args, Users[source])
                ...
                if (not (command.arguments <= (#command_args - 1)) and command.arguments > -1) then
                    ...
                    command.cmd(source, command_args, Users[source])
                    TriggerEvent("trp:userCommandRan", source, command_args)
                end
            end
            TriggerEvent("trp:commandRan", source, command_args, Users[source])
        else
            TriggerEvent('trp:invalidCommandHandler', source, command_args, Users[source])
        ...
        TriggerEvent('trp:chatMessage', source, message, Users[source])
        if WasEventCanceled() then
            CancelEvent()
        end
    end
...
AddEventHandler('trp:addCommand', function(command, callback, suggestion, arguments)
    ...
    if (settings.defaultSettings.disableCommandHandler ~= 'false') then
        RegisterCommand(command, function(source, args)
            ...
            if (source ~= 0) then
                ...
                if ((#args <= commands[command].arguments and #args == commands[command].arguments, #args, Users[source]) or commands[command].arguments == -1) then
                    callback(source, args, Users[source])
                else
                    TriggerEvent("trp:incorrectAmountOfArguments", source, commands[command].arguments, #args, Users[source])
                end
...
AddEventHandler('trp:addAdminCommand', function(command, perm, callback, callbackfailed, suggestion, arguments)
...
function addGroupCommand(command, group, callback, callbackfailed, suggestion, arguments)
    ...
    ExecuteCommand('add_ace_group.' .. group .. ' command.' .. command .. ' allow')
...
AddEventHandler('trp:addGroupCommand', function(command, group, callback, callbackfailed, suggestion, arguments)
    addGroupCommand(command, group, callback, callbackfailed, suggestion, arguments)
end)

AddEventHandler('trp:addACECommand', function(command, group, callback)
    addACECommand(command, group, callback)
end)

RegisterServerEvent('trp:updatePositions')
AddEventHandler('trp:updatePositions', function(x, y, z)
    if (Users[source]) then
...
commands['info'].cmd = function(source, args, user)
    local Source = source
    TriggerClientEvent('chatMessage', Source, 'SYSTEM', { 255, 0, 0 }, "^2[^3TRP-FRAMEWORK^2]^0 Version: ^2 " .. _VERSION)
    TriggerClientEvent('chatMessage', Source, 'SYSTEM', { 255, 0, 0 }, "^2[^3TRP-FRAMEWORK^2]^0 Commands Loaded: ^2 " .. (returnIndexesInTable(commands) - 2))
end
...
commands["devinfo"].cmd = function(source, args, user)
    ...
    if GetConvar('trp_enableCustomData', 'false') == "1" then
        db = "Custom"
    end
    TriggerClientEvent('chatMessage', Source, 'SYSTEM', { 255, 0, 0 }, "^2[^3TRP-FRAMEWORK^2]^0 Version: ^2 " .. _VERSION))
    TriggerClientEvent('chatMessage', Source, 'SYSTEM', { 255, 0, 0 }, "^2[^3TRP-FRAMEWORK^2]^0 Groups: ^2 " .. (returnIndexesInTable(groups) - 1))
    TriggerClientEvent('chatMessage', Source, 'SYSTEM', { 255, 0, 0 }, "^2[^3TRP-FRAMEWORK^2]^0 Commands loaded: ^2 " .. (returnIndexesInTable(commands) - 1))
    TriggerClientEvent('chatMessage', Source, 'SYSTEM', { 255, 0, 0 }, "^2[^3TRP-FRAMEWORK^2]^0 Database: ^2 " .. db)
    TriggerClientEvent('chatMessage', Source, 'SYSTEM', { 255, 0, 0 }, "^2[^3TRP-FRAMEWORK^2]^0 Logging enabled: ^2 " .. tostring(settings.defaultSettings.enableLogging))
end
